...
public class CharacterBuilderControlPanel extends ControlPanel {
	...
	public void next() {
		...
		if (mainComponents.get(pageIndex) instanceof SocialMainComponent
				&& ((SocialMainComponent) mainComponents.get(pageIndex)).isEmpty()) {
			next();
		}
	}
...
//			addActorToActorsList(actor);
			...
			JOptionPane.showMessageDialog(null,
					"Your informations were successfully saved! Don't forget to commit the changes and please do a quick check for any errors!");
		} catch (Exception e) {
			PostInfoXmlGenerationRunnableHolder.clear();
			JOptionPane.showMessageDialog(null,
	...
	private boolean checkItemEditingEnabled(CharacterBuilderMainComponent mainComponent) {
		...
		if (((EquipmentCreatorMainComponent) mainComponent).isEditingModeEnabled()) {
			JOptionPane.showMessageDialog(null, "You need to finish editing all items before proceeding!", "Error",
	...
	private void addActorToActorsList(Actor actor) throws IOException {
		...
		String sourceString = ValueFormatter.isEmpty(source.getInstallment()) ? source.getFranchise()
	...
	private void addEquipmentToEquipmentList(Actor actor) throws IOException {
		...
		File file = GameFileAccessor.getFileFromProperty(PropertyRepository.EQUIPMENT_LIST);
		File tempFile = new File(file.getParentFile(), file.getName() + ".tmp");
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(file));
			...
			equipments.stream().forEach(e -> writer.println(ValueFormatter.formatListEntry(e)));
		}
	...
	private void verifyActor(Actor actor, List<String> errors) {
		if (actor.getName() == null || ValueFormatter.isEmpty(actor.getName().getFirst())
				&& ValueFormatter.isEmpty(actor.getName().getLast())) {
			errors.add("Enter at least a first or a last name.");
		}
		if (actor.getName() != null && !ValueFormatter.isEmpty(actor.getName().getMiddle())
				&& (ValueFormatter.isEmpty(actor.getName().getFirst())
						|| ValueFormatter.isEmpty(actor.getName().getLast()))) {
	...
	private String getInstallmentDirFromUserInput(String selectedInstallment) {
		String input = JOptionPane.showInputDialog(
				"Please enter a valid directory name for the installment directory (i.e. remove special characters).",
				selectedInstallment);
	...
	private File getCharacterFolder(String franchiseDirName, String installmentDirName, String fullCharacterName) {
		File parentDir = new File(GameFileAccessor.getFileFromProperty(PropertyRepository.ACTORS_FOLDER),
				franchiseDirName);
		if (!parentDir.exists()) {
			parentDir.mkdir();
		}
	...
	public void init(List<CharacterBuilderComponent> components) {
		...
		statGenerator = (StatGenerator) mainComponents.stream().filter(c -> c instanceof StatGenerator).findFirst()
				.get();
